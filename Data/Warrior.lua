...
function Warrior:init()
	...
	FeralFunctions = {
		FerociousBite = function(data, rank)
			...
			data.damage = comboDamageByRank[rank][combo] + (1.5 * (energy-35)) * 1+(combo*(attackPowerBonus()*0.03))
		end,
	...
	addFunctionsToSpells(ArmsFunctions, WarriorSpells.Arms)
	addFunctionsToSpells(FuryFuntions, WarriorSpells.Fury)
	addFunctionsToSpells(ProtectionSpells, WarriorSpells.Protection)
end